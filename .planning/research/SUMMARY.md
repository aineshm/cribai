...
Space Grotesk and DM Sans are both available on Google Fonts and can be loaded directly via `next/font/google` with zero manual download. Use `import { Space_Grotesk, DM_Sans } from 'next/font/google'` in `layout.tsx`. TanStack Query is recommended (not yet confirmed) for the AI Concierge mission polling use case to prevent state race conditions; this is the one open dependency decision.
...
- `next/font/google`: Space Grotesk + DM Sans delivery — both available on Google Fonts, zero manual download
...
**Delivers:** Space Grotesk + DM Sans via `next/font/google` (both on Google Fonts), shadcn/ui initialized with token bridge in `globals.css`, `tw-animate-css` replacing `tailwindcss-animate`, `optimizePackageImports` for Lucide, verified build and test pipeline.
**Addresses:** Design system consistency (table stakes), font loading (prerequisite)
**Avoids:** CSS variable collision (Pitfall 2), big-bang migration (Pitfall 1), font loading misconfiguration (Pitfall 4), Tailwind v4 build breakage (Pitfall 10), Lucide bundle bloat (Pitfall 5)
...
**Avoids:** framer-motion client boundary proliferation (Pitfall 3 — establish `MotionWrapper` pattern here for all future phases)
...
**Avoids:** floating panel losing context on navigation (Pitfall 9 — layout-level panel with global state)
...
**Avoids:** HITL draft failure modes (Pitfall 7 — `draft_version`, `expires_at`, and idempotency key must be in the schema from the start, never retrofitted)
...
**Avoids:** mission executor Vercel timeout (Pitfall 6 in ARCHITECTURE.md Anti-Pattern 3 — API returns 202 immediately, executor is fire-and-forget)
...
**Avoids:** mission state polling race condition (Pitfall 6 — TanStack Query, no raw `setInterval`), HITL draft safety guardrails (Pitfall 7), steering bar ambiguous intent (Pitfall 8 — `unknown_intent` handler with visible clarification UI)
...
|------|------------|-------|
| Stack | HIGH | All core libraries verified against official docs. shadcn/ui Tailwind v4 support confirmed. motion v12 React 19 compatibility confirmed. Font delivery via `next/font/google` — both Space Grotesk and DM Sans available on Google Fonts. |
...
| Pitfalls | HIGH | CSS token collision, framer-motion boundary, font loading configuration, Lucide bundle, and all three HITL failure modes are verified from official docs, GitHub issues, and community post-mortems with HIGH confidence. |
