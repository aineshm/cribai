...
- [ ] **AUTH-01**: User can sign in via magic link email and land on authenticated experience without redirect errors
- [ ] **AUTH-02**: User session persists across browser refresh and tab close/reopen
- [ ] **AUTH-03**: System validates that user email is a .edu address at signup (client-side check, not full verification)
...
- [ ] **SRCH-01**: Listings are embedded with Gemini gemini-embedding-001 and stored as pgvector columns for semantic search
...
- [ ] **LIST-01**: User can save/favorite listings and view them from a saved listings page
...
- [ ] **DATA-05**: Nightly scrape automation runs via GitHub Actions with monitoring/alerting on failures
